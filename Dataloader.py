      ...
      if self.augmentation:
          if random.randint(0, 1) == 1:
            #natural noise
              ...
              audio_noise, sr = load_file(n_noise)
              final_audio_noise  = time_elong(sr, audio_noise, m_time)
              snr = np.random.randint(-10, 0)
          else:
            #gaussian noise
              n_noise = np.random.normal(loc=0, scale=1, size=(1, m_time*sr))
              final_audio_noise = librosa.to_mono(n_noise)
              snr = np.random.randint(0, 50)  #-10/5 for natural noise, 
